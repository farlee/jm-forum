...
class Backup extends Injectable
{
    public function generate()
    {
        ...
        system('/usr/bin/mysqldump -u ' . $config->username . ' -p' . $config->password . ' -r /tmp/Jimi.sql ' . $config->dbname);
        system('bzip2 /tmp/Jimi.sql');

        $sourcePath = '/tmp/Jimi.sql.bz2';
        ...
        $client = new Client($accessToken, "Jimi", null, $host);

        $dropboxPath = '/Jimi.sql.bz2';
